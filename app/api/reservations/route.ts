...
export async function POST(request: Request) {
  try {
    ...
    if (
      ...
      console.warn('Appwrite database details are placeholders. Returning mock successful response.');
      return NextResponse.json(
        {
    ...
    return NextResponse.json(
      { error: error.message || 'An error occurred while saving the reservation' },
      { status: 500 }
    );
  }
}
